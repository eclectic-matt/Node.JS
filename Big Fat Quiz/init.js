...
const handleGet = (req) => {
  ...
  if(page.indexOf('/') === false && page !== 'view'){
    console.log(getDateNow(), "\tGET\t", page);
  }
  ...
  switch(page){
    ...
    break;
		...
		//============
		// JSON, JS, CSS FILES
    // NOTE: CLEARING OUTPUT AND ONLY RETURNING THAT DATA
		//============
    ...
    case 'style/canvas.css':
      output = fs.readFileSync('./pages/style/canvas.css', 'utf8');
		break;
    case 'style/view.css':
      output = fs.readFileSync('./pages/style/view.css', 'utf8');
    break;
    case 'resource/QRjoin.PNG':
      //NO LONGER USED - ENCODED AS A BASE64 STRING IN THE FILE
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    //GET REQUEST - HANDLE NON-HTML ELEMENTS
