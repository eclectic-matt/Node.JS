<?php

$mysqli = new mysqli("localhost","root","","big_fat_quiz");

//HARD-CODED
$game = 1;

//CHECK CONNECTION
if($mysqli->connect_errno){
	echo "Failed to connect to MySQL: " . $mysqli -> connect_error;
	exit();
}

echo '<!DOCTYPE html><head><meta http-equiv="refresh" content="5">';
echo '<link rel="stylesheet" href="./style/view.css" type="text/css" />
<title>Big Fat Quiz</title>
</head>
<body>
<h1>Big Fat Quiz</h1>
<div id="teamsBox">
';


//PERFORM QUERY
if($teams = $mysqli->query("SELECT * FROM team WHERE game = " . $game)){
	foreach($teams as $team){
		/*echo 'TEAM: ' . $team['name'] . '<br>';
		if($canvases = $mysqli->query("SELECT * FROM canvas WHERE team = " . $team['id'])){
			echo 'CANVASES COUNT: ' . $canvases->num_rows . '<br>';
		}*/
		echo generateTeamBox($team);
	}
	//echo "Returned rows are: " . $result -> num_rows;
	// Free result set
	//$teams->free_result();
}


die();


header("Expires: Mon, 01 Jan 2024 06:00:00 GMT");
header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
header("Cache-Control: no-store, no-cache, must-revalidate, max-age=0");
header("Cache-Control: post-check=0, pre-check=0", false);
header("Pragma: no-cache");
header("Connection: close");

$opts = [
	'http' => [
		'header' => "Cache-Control: no-cache\r\n" .
			"Pragma: no-cache\r\n" . 
			"Accept-language: en\r\n" .
			"Cookie: foo=bar\r\n"
	]
];
$context = stream_context_create($opts);

//READ ADMIN.JSON TO GET CURRENT VIEW
$adminPath = __DIR__ . '/data/admin.json';
//$adminJson = file_get_contents($adminPath);
//$adminJson = file_get_contents($adminPath . '?v=' . date("Ymdhis"));
$adminJson = file_get_contents($adminPath, false, $context);
$adminData = json_decode($adminJson);
$view = $adminData->view;
//echo 'CURRENT VIEW: ' . $view;

//READ TEAMS.JSON TO GET CURRENT TEAMS 
$teamsPath = __DIR__ . '/data/teams.json';
//$teamsJson = file_get_contents($teamsPath);
//$teamsJson = file_get_contents($teamsPath . '?v=' . date("Ymdhis"));
$teamsJson = file_get_contents($teamsPath, false, $context);
$teamsData = json_decode($teamsJson);

//GET CANVAS MANAGER CLASS 
require_once('./class/CanvasManager.php');
$cnvMgr = new \classes\CanvasManager();

$output = '<!DOCTYPE html><head><meta http-equiv="refresh" content="5">';
$output .= '<link rel="stylesheet" href="./style/view.css" type="text/css" />
<title>Big Fat Quiz</title>
</head>';


switch($view){
	case 'welcome':
		//START OUTPUT
		$join = '<body>
			<div id="headerRow" class="clearfix">
				<h1>Big Fat Quiz 
				<img 
					src="data:image/png;base64, " 
					width="150" 
					height="150" 
				/>
				</h1>
			</div>
			<div id="teamsBox">';

			for($i = 0; $i < count($teamsData); $i++){
				$team = $teamsData[$i];
				$encName = htmlentities($team->name);
				$join .= '<div id="' . $encName . '" class="teamBox">';
				$join .= '<h2 class="teamHead">';
					$join .= '<span id="' . $encName . 'Name">' . $team->name . '</span> ';
					$join .= '<span class="score" id="' . $encName . 'Score">' . $team->score . '</span>';
				$join .= '</h2>';
				//CHECK FOR A CURRENT CANVAS FOR THIS ROUND
				$cnv = $cnvMgr->getTeamCanvas($team->name, 0, 0);
				if($cnv !== false){
					$cnvPng = $cnv->data;
					$join .= '<img
						src="' . $cnvPng . '"
						width="200"
						height="auto"
						/>';
				}else{
					$join .= '<em>NO CANVAS FOUND!</em>';
				}

				$join .= '</div>';
			}

			$join .= '</div>
		</body>';

		$output .= $join;
	break;
	case 'questions':
		$questions = '<h1>Questions</h1>';
		$output .= $questions;
	break;
}

echo $output;


function generateTeamBox($team){
	global $mysqli;
	$teamBoxStr = '';
	$encName = htmlentities($team['name']);
	$teamBoxStr .= '<div id="' . $encName . '" class="teamBox">';
	$teamBoxStr .= '<h2 class="teamHead">';
		$teamBoxStr .= '<span id="' . $encName . 'Name">' . $team['name'] . '</span> ';
		//GET SCORE 
		$scores = $mysqli->query('SELECT * FROM score WHERE team = ' . $team['id']);
		if($scores->num_rows > 0){
			$teamScore = 0;
			foreach($scores as $score){
				$teamScore += $score['points'];
			}
		}else{
			$teamScore = 0;
		}
		$teamBoxStr .= '<span class="score" id="' . $encName . 'Score">' . $teamScore . '</span>';
	$teamBoxStr .= '</h2>';

	//CHECK FOR A CURRENT CANVAS FOR THIS ROUND
	//$cnv = $cnvMgr->getTeamCanvas($team->name, 0, 0);
	$canvas = $mysqli->query('SELECT * FROM canvas WHERE team = ' . $team['id'] . ' AND round=0 AND question=0');
	if($canvas->num_rows > 0){
		//var_dump($canvas);
		$canvasData = mysqli_fetch_assoc($canvas);
		$teamBoxStr .= '<img
			src="' . $canvasData['data'] . '"
			width="200"
			height="auto"
			/>';
	}else{
		$teamBoxStr .= '<em>NO CANVAS FOUND!</em>';
	}

	$teamBoxStr .= '</div>';
	return $teamBoxStr;
}



//CLOSE MYSQL CONNECTION
$mysqli->close();