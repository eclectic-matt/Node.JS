...
client.on('message', message => {
  ...
  */
  ...
  /*
      --- NEXT COMMAND
  */
  }else if (message.content === '!cn pass'){
  ...
  */
    ...
    if (foundIndex !== -1){
      ...
      */
      ...
      // Send the update word grid to the spymasters
      // Get a variable holding the user sm1
      const sm1user = client.users.cache.get(sm1);
      // Send just the new word grid to sm1
      sm1user.send(thisCurrentWords + '\n\n');
      // Send a break to space it out
      sm1user.send('\n\n \n\n');
      // Get a variable holding the user sm2
      const sm2user = client.users.cache.get(sm2);
      // Send just the new word grid to sm2
      sm2user.send(thisCurrentWords + '\n\n');
      // Send a break to space it out
      sm2user.send('\n\n \n\n');

      //client.sendMessage({to: sm1, message: thisCurrentWords});
      //client.sendMessage({to: sm2, message: thisCurrentWords});
      //client.users.get(sm1).send(thisCurrentWords);
      //client.users.get(sm2).send(thisCurrentWords);
