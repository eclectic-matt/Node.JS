...
//https://stackoverflow.com/a/49583416
...
class CodenamesGame {
  ...
  */
  constructor (channel){
    ...
    console.log(getDateStamp(),' New Channel added - "' + channel.name + '"');
    ...
    this.grid = {};
    // The number of rotations taken on this spy grid
    this.grid.rotation = 0;
  ...
  */
  initialiseWordGrid(){
    ...
    // OLD VERSION - Not rotated
    //this.grid.spyGrid = SPY_GRIDS[this.grid.gridTemplate];

    // Rotate 0 - 3 times
    this.grid.rotation = Math.floor(Math.random() * 3);

    let theInitialGrid = SPY_GRIDS[this.grid.gridTemplate];

    // Removes the first element and modifies
    let theFirstTeam = theInitialGrid.shift();
    // theInitialGrid is now just the team grid
    if (this.grid.rotation !== 0){
      for (let i = 0; i < this.grid.rotation; i++){
        theInitialGrid = rotateArray(theInitialGrid);
      }
    }

    // Now assign this back to a new array for the rest of the code
    this.grid.spyGrid[0] = theFirstTeam;
    for (let i = 0; i < theInitialGrid.length; i++){
      this.grid.spyGrid.push(theInitialGrid[i]);
    }

    // Testing - check the current grid
    console.log('New spy grid',this.grid.spyGrid);
...
// Only works for NxN arrays
function rotateArray(array){
  console.log('Rotating ',array);
  // Get the number of rows
	let arrayR = array.length;
  // Get the number of columns
	let arrayC = array[0].length;
  // Initialise the new 2d array
  let newArr = [];
  for (let i = 0; i < arrayR; i++){
    // Push an array into the new array
    newArr.push ([]);
  }
  // For each row of the old array
 	for (let i = 0; i < arrayR; i++){
      // For each column in this row
      for (let j = 0; j < arrayC; j++){
        // This shifts by column
        newArr[j][arrayC - 1 - i] = array[i][j];
      }
    }
  return newArr;
}
...
client.on('message', message => {
  ...
  */
    ...
    console.error('ISSUE BEING LOGGED!');
    console.error('-----------------------------------------------');
    console.error(getDateStamp(),'New Issue raised by channel "' + message.channel.name + '"');
    ...
    if (foundIndex !== -1){
        ...
        */
        if (thisTeam === ASSASSIN_GUESS){
          ...
          console.log(getDateStamp(),'Game Ended in Channel - "' + message.channel.name + '"');
